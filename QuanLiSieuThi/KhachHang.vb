...
Public Class KhachHang
    ...
    Private Sub LuuBtn_Click(sender As Object, e As EventArgs) Handles LuuBtn.Click
        If MsgBox("Bạn có muốn thêm không? ", vbYesNo, "Đã thêm") = MsgBoxResult.Yes Then
            ...
            sdtText.Text = "" Or mailText.Text = "" Then
                MsgBox("Chưa nhập đủ")
            Else
                lenh = "insert into KhachHang(maKH,tenKH, diaChiKH, SDT, Email) values(dbo.fn_autoIDKH(),N'" & nameText.Text & "',
                        N'" & dChiText.Text & "','" & sdtText.Text & "','" & mailText.Text & "')"
                Dim cmd As New SqlCommand(lenh, con)
                con.Open()
                cmd.ExecuteNonQuery()
                con.Close()
    ...
    Private Sub SuaBtn_Click(sender As Object, e As EventArgs) Handles SuaBtn.Click
        If MsgBox("Bạn có muốn sửa không? ", vbYesNo, "Đã sửa") = MsgBoxResult.Yes Then
            ...
            Else
                ...
                lenh = "update  KhachHang  set tenKH = N'" & nameText.Text & "',diaChiKH = N'" & dChiText.Text & "',SDT = '" &
    ...
    Public Sub FilterData(valueToSearch As String)
        ...
        If cbSearch.Text = "Mã khách hàng" Then
            Dim searchQuery As String = "SELECT * From KhachHang WHERE maKH like '%" & valueToSearch & "%'"
            Dim cmd As New SqlCommand(searchQuery, con)
            Dim adapter As New SqlDataAdapter(cmd)
            adapter.Fill(table)
        ElseIf cbSearch.Text = "Tên khách hàng" Then
            Dim searchQuery As String = "SELECT * From KhachHang WHERE tenKH like '%" & valueToSearch & "%'"
            Dim cmd As New SqlCommand(searchQuery, con)
            Dim adapter As New SqlDataAdapter(cmd)
            adapter.Fill(table)
        ElseIf cbSearch.Text = "Địa chỉ" Then
            Dim searchQuery As String = "SELECT * From KhachHang WHERE diaChiKH like '%" & valueToSearch & "%'"
            Dim cmd As New SqlCommand(searchQuery, con)
            Dim adapter As New SqlDataAdapter(cmd)
            adapter.Fill(table)
        ElseIf cbSearch.Text = "Email" Then
            Dim searchQuery As String = "SELECT * From KhachHang WHERE Email like '%" & valueToSearch & "%'"
            Dim cmd As New SqlCommand(searchQuery, con)
            Dim adapter As New SqlDataAdapter(cmd)
            adapter.Fill(table)
        End If
    ...
    Private Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        e.Graphics.DrawString("BÁO CÁO KHÁCH HÀNG", New Drawing.Font("Time New Roman", 16), Brushes.Black, 300, 20)
        ...
        e.Graphics.DrawImage(imagebmp, 70, 100)
        e.Graphics.DrawString("Ngày xuất báo cáo: " + Date.Now.ToString(), New Drawing.Font("Time New Roman", 10), Brushes.Black, 550, 450)
